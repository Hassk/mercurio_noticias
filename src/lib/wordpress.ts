...
interface EmbeddedPage extends Page {
  _embedded?: {
    'wp:featuredmedia'?: Array<{
      source_url: string;
      // Otros campos relevantes del media
    }>;
  };
}
...
export async function getPageBySlug(slug: string): Promise<EmbeddedPage> {
  const url = getUrl("/wp-json/wp/v2/pages", { slug, _embed: true });
  const response = await wordpressFetch<EmbeddedPage[]>(url, {
    next: {
      ...defaultFetchOptions.next,
